 ...
 */
@Component
public class PddGetMissOrderJob implements Job {
	
	private Logger LOGGER = LoggerFactory.getLogger(PddGetMissOrderJob.class);
		...
		for (CatchOrderConfig grabConfig : orderGrabConfigList) {
			try {
				if (grabConfig.getStoreId() != null) {
					messageSender.send(QueueEnum.PDD_GET_MISS_ORDER,new BasicMessage(grabConfig.getId(), grabConfig.getStoreCode(), String.valueOf(executeTime.getTime()), ""));
				}
			} catch (Exception e) {
				LOGGER.error(String.format("拼多多漏单抓单异常!抓单配置 %s，异常信息:%s", grabConfig.getStoreId(), e.getMessage()),e);
			}
