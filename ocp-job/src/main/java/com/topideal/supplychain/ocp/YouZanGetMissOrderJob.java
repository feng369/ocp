...
/**
 * @Author: Hu Yong
 * @Description: 有赞抓取漏掉订单
 * @Date: Created In 2019/5/5 10:33
 * @Modified By:
 */
@Component
public class YouZanGetMissOrderJob implements Job {
	
	private Logger logger = LoggerFactory.getLogger(YouZanGetMissOrderJob.class);
		...
		for (CatchOrderConfig grabConfig : orderGrabConfigList) {
			try {
				if (grabConfig.getStoreId() != null) {
					messageSender.send(QueueEnum.OCP_YOUZAN_GET_MISS_ORDER,new BasicMessage(grabConfig.getId(),grabConfig.getStoreCode()));
				}
			} catch (Exception e) {
				logger.error(String
