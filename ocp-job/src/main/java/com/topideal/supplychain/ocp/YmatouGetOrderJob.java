 ...
 */
@Component
public class YmatouGetOrderJob implements Job {
	
	private Logger logger = LoggerFactory.getLogger(YmatouGetOrderJob.class);
	...
	public void execute(JobExecutionContext jobExecutionContext) {
		//拿到ymatou电商平台编码
		String ymatou = PlatformEnum.YMATOU.getCode();
		List<Platform> platforms = platformService.findByVirtualCode(ymatou);
		...
		for (CatchOrderConfig grabConfig : orderGrabConfigList) {
			try {
				if (grabConfig.getStoreId() != null) {
					messageSender.send(QueueEnum.OCP_YMATOU_GRAB_ORDER,new BasicMessage(grabConfig.getId(),grabConfig.getStoreCode()));
				}
			} catch (Exception e) {
				logger.error(String
					.format("洋码头抓单异常!抓单配置 %s，异常信息:%s", grabConfig.getStoreId(), e.getMessage()),e);
			}
