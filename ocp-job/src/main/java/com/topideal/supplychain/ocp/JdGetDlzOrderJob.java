...
public class JdGetDlzOrderJob implements Job {
    private Logger LOGGER = LoggerFactory.getLogger(JdGetDlzOrderJob.class);
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        ...
        List<Platform> platforms = platformService.findByVirtualCode(PlatformEnum.JD_DLZ.getCode());
        ...
        for (CatchOrderConfig grabConfig : orderGrabConfigList) {
            try {
                messageSender.send(QueueConstants.QueueEnum.OCP_JD_DLZ_GET_ORDER, new BasicMessage(grabConfig.getId(), grabConfig.getCode()));
            }catch (Exception e) {
                LOGGER.error(String
                        .format("京东多渠道独立站抓单异常!抓单配置 %s，异常信息:%s", grabConfig.getCode(), e.getMessage()),e);
            }
        }
