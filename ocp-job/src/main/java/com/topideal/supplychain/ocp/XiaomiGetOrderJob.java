 ...
 */
@Component
public class XiaomiGetOrderJob implements Job {
	
	private Logger logger = LoggerFactory.getLogger(XiaomiGetOrderJob.class);
	...
	public void execute(JobExecutionContext jobExecutionContext) {
		//拿到baoma电商平台编码
		String baoma = PlatformEnum.XIAOMI.getCode();
		...
		for (CatchOrderConfig grabConfig : orderGrabConfigList) {
			try {
				if (grabConfig.getStoreId() != null) {
					messageSender.send(QueueEnum.OCP_XIAOMI_GET_ORDER,new BasicMessage(grabConfig.getId(),grabConfig.getStoreCode()));
				}
			} catch (Exception e) {
				logger.error(String
					.format("小米抓单异常!抓单配置 %s，异常信息:%s", grabConfig.getStoreId(), e.getMessage()),e);
			}
