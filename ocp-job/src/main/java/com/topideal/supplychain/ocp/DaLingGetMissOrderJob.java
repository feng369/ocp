 ...
 **/
@Component
public class DaLingGetMissOrderJob implements Job {

    private Logger logger = LoggerFactory.getLogger(DaLingGetMissOrderJob.class);
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        ...
        for (CatchOrderConfig grabConfig : orderGrabConfigList) {
            try {
                messageSender.send(QueueConstants.QueueEnum.OCP_DALING_GET_MISS_ORDER, new BasicMessage(grabConfig.getId(), grabConfig.getStoreCode()));
            } catch (Exception e) {
                logger.error(String
