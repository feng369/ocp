 ...
 */
//@Component
public class BeibeiGetOrderJob implements Job {
	
	private Logger logger = LoggerFactory.getLogger(BeibeiGetOrderJob.class);
	...
	public void execute(JobExecutionContext jobExecutionContext) {
		//拿到贝贝电商平台编码
		String beibeiCode = PlatformEnum.BEIBEI.getCode();
		List<Platform> platforms = platformService.findByVirtualCode(beibeiCode);
		...
		for (CatchOrderConfig grabConfig : orderGrabConfigList) {
			try {
				if (grabConfig.getStoreId() != null) {
					messageSender.send(QueueEnum.OCP_BEIBEI_GET_ORDER,new BasicMessage(grabConfig.getId(),grabConfig.getStoreCode()));
				}
			} catch (Exception e) {
				logger.error(String
					.format("贝贝抓单异常!抓单配置 %s，异常信息:%s", grabConfig.getStoreId(), e.getMessage()),e);
			}
